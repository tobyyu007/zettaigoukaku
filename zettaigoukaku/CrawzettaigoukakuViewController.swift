//
//  CrawzettaigoukakuViewController.swift
//  zettaigoukaku
//
...
class CrawzettaigoukakuViewController: NSViewController, WKNavigationDelegate {

    @IBOutlet weak var crawzettaigoukakuWebView: WKWebView!
    ...
    override func viewDidLoad() {
        ...
        if let url = URL(string: "https://www.mojidict.com") {
            crawzettaigoukakuWebView.load(URLRequest(url: url))
        }
        crawzettaigoukakuWebView.navigationDelegate = self
        crawzettaigoukakuWebView.addObserver(self, forKeyPath: "URL", options: .new, context: nil) // 偵測頁面改變
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        // 將頁面滑到底部
        crawzettaigoukakuWebView.evaluateJavaScript(javascriptString){ (value, error) in
            if let err = error {
                print(err)
            }
        }
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if let key = change?[NSKeyValueChangeKey.newKey] {
            ...
            addzettaigoukakuViewController.progress = 10 // stringValue: 頁面載入中
            addzettaigoukakuViewController.webViewEnd = true
            self.dismiss(CrawzettaigoukakuViewController.self) // 輸入完單字後就關閉 webView 視窗
        }
    }
    ...
    func crawData(webURL: String)
    {
        AF.request(webURL).responseString{ response in
            if let html = response.value{
                addzettaigoukakuViewController.progress = 20 // stringValue: 抓取資料中
                self.parsehtml(html)
                //print(html)
            }
        }
    ...
    func parsehtml(_ html1: String)
    {
        ...
        for volcabulary in doc!.xpath("/html/body/div/div/div/div[2]/div[1]/div[1]/h3/span")
        {
            ...
            CrawzettaigoukakuViewController.VD.volcabulary = volcabulary.text!
        }
        ...
        for kana in doc!.xpath("//div[@class='pron jp-font']")
        {
            ...
            print(matched[0][0])
            CrawzettaigoukakuViewController.VD.kana = matched[0][0]
        }
        ...
        CrawzettaigoukakuViewController.VD.type = resultString.components(separatedBy: "·")
        print(CrawzettaigoukakuViewController.VD.type)
        ...
        CrawzettaigoukakuViewController.VD.chineseDefenition = chineseDefinitionResult
        CrawzettaigoukakuViewController.VD.japaneseDefenition = japaneseDefinitionResult
        ...
        CrawzettaigoukakuViewController.VD.sentence = sentenceResult
        ...
        CrawzettaigoukakuViewController.VD.sentence_chinese = chineseSentenceResult
        addzettaigoukakuViewController.progress = 30 // stringValue: 抓取完成
        print(chineseSentenceResult)
    }
    
    @IBAction func cancel(_ sender: Any) {
        // "取消" 按鍵
        self.dismiss(CrawzettaigoukakuViewController.self)
        addzettaigoukakuViewController.webViewEnd = true
        addzettaigoukakuViewController.progress = -1
    }
}
